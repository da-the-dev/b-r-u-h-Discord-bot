    ...
    async exec(msg, args, client) {
        ...
        if (info) {
            await msg.guild.channels.get(info.textid).delete('Group deletion.')
                .then(channel => {
                    console.log("User", msg.author.username, "deleted the TEXT channel of the group:", channel.id)
                })
            await msg.guild.channels.get(info.voiceid).delete('Group deletion')
                .then(channel => {
                    console.log("User", msg.author.username, "deleted the VOICE channel of the group:", channel.id)
                })
        }
